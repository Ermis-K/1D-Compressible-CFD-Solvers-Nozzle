...
rho((1.4999999<x)&(x<3.11111111))=0.634-0.3879*(x((1.4999999<x)&(x<3.11111111))-1.5);

T((-1<x)&(x<0.5))=1;
T((0.4999999999<x)&(x<1.5))=1-0.167*(x((0.4999999999<x)&(x<1.5))-0.5);
T((1.4999999<x)&(x<3.11111111))=0.833-0.3507*(x(((1.4999999<x)&(x<3.11111111)))-1.5);
...
while step<total_steps+1
    ...
    U3(end)=2*U3(end-1)-U3(end-2);





    rho_15(step)=rho(find(A == 1));
    T_15(step)=T(find(A == 1));
    P_15(step)=P(find(A == 1));
    M_15(step)=M(find(A == 1));

    dU1dt_avg_16(step)=dU1dt_avg(find(A == 1));
    dU2dt_avg_16(step)=dU2dt_avg(find(A == 1));
    dU3dt_avg_16(step)=dU3dt_avg(find(A == 1));
...
for i = 1:N
    if abs(A(i) - 1) < 1e-6 
        ...
        M_analytical(i, 2) = fzero(eqn, 2.0);
    end
end
...
subplot(4,1,1);
plot(rho_15)
title('Non-dimensionalized ρ at point A=1');
xlabel('Time step')
grid on;
subplot(4,1,2);
plot(T_15)
title('Non-dimensionalized T at point A=1'); 
xlabel('Time step')
grid on;
subplot(4,1,3);
plot(P_15)
title('Non-dimensionalized P at point A=1'); 
xlabel('Time step')
grid on;
subplot(4,1,4);
plot(M_15)
title('Non-dimensionalized M at point A=1'); 
xlabel('Time step')
grid on;
